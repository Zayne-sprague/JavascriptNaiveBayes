        ...
        return (
            <div className={join('ff-nn-layer', className)}>
                {_.map(_.get(layer, 'nodes'), (n, i)=>{
                    return (
                        <FfNnNode key={`node-${i}`} label={_.get(layer, `labels[${i}]`)} activation_weight={n} color={_.get(layer, 'color')} shape={_.get(layer, 'shape')} correct_label={_.includes(_.get(layer, 'correct_labels', []), _.get(layer, `labels[${i}]`))}/>
                    )
                })}
            </div>
        );
